...
Publishes ``uav_msgs/UavTelemetry`` on ``/data`` at ``publish_hz`` Hz.
The asyncio event loop runs on its own daemon thread so it never blocks
the ROS 2 executor, and the MAVSDK connection auto-reconnects on error.
"""
...
class TelemetryNode(Node):
    def __init__(self) -> None:
        ...
        self._timer = self.create_timer(1.0 / max(0.1, hz), self._publish)
    ...
    async def _connect_loop(self) -> None:
        url = self.get_parameter("connection_url").get_parameter_value().string_value
        while rclpy.ok():
            self.get_logger().info(f"Connecting to ArduPilot at {url}")
            system = System(port=50052)
            try:
